...
async def test_get_rooms_by_hotel_id_and_dates(hotel_id, date_from, date_to, status_code, ac: AsyncClient):
    response = await ac.get(f"/hotels/{hotel_id}/rooms", params={
        'date_from': date_from,
        'date_to': date_to
    })
    assert response.status_code == status_code
